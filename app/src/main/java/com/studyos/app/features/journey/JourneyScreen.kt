...
import com.studyos.app.features.journey.components.DuolingoPathNode
import com.studyos.app.features.journey.components.LessonBottomSheet
...
fun JourneyScreen(
    ...
    var selectedSubjectId by remember(defaultSubjectId) {
        mutableStateOf(defaultSubjectId)
    }

    LaunchedEffect(defaultSubjectId) {
        if (currentSemester.subjects.none { it.id == selectedSubjectId }) {
            ...
            onMarkComplete = { lessonToMark ->
                ...
                if (currentUser != null) {
                    scope.launch {
                        ...
                        progressionRepo.completeTopic(currentUser.uid, scopedKey, TopicDifficulty.MEDIUM)
